...
public class Operations {
     ...
     */
    private static final WeakHashMap<Grammar, Map<String, Set<String>>> CACHED_FIRST =
        new WeakHashMap<>();
    /**
     * The cached follow sets, maps from grammars to follow sets.
     */
    private static final WeakHashMap<Grammar, Map<String, Set<String>>> CACHED_FOLLOW =
        new WeakHashMap<>();
    /**
     * The cached variables to productions maps, maps from grammars to maps from
     * variables to productions on that variable.
     */
    private static final WeakHashMap<Grammar, Map<String, Set<Production>>> CACHED_VPMAP =
        new WeakHashMap<>();
     ...
     *
     * @param grammar the jflap.grammar to calculate first sets for
     ...
     *
     * @param grammar the jflap.grammar to calculate follow sets for
     ...
     *
     * @param grammar the jflap.grammar to test
     ...
     *
     * @param grammar the jflap.grammar to augment
     ...
     *
     * @param grammar the jflap.grammar for which this production is taking place
     ...
     *
     * @param grammar the jflap.grammar to calculate goto on
    ...
    public static Set<Production> goTo(Grammar grammar, Set<Production> items, String symbol) {
        ...
        while (it.hasNext()) {
            ...
            String newRhs = item.getRHS().substring(0, p - 1)
                + item.getRHS().charAt(p) + ITEM_POSITION
     ...
     *
     * @param grammar the jflap.grammar to get a map for
    ...
    public static Map<String, Set<Production>> getVariableProductionMap(Grammar grammar) {
        if (CACHED_VPMAP.containsKey(grammar)) {
            return Collections
                .unmodifiableMap(CACHED_VPMAP.get(grammar));
        }
