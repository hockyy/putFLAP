...
public class CYKTracer {
    ...
    private void initializeLambdaStepMap() {
        ...
        if (lambdaDerivers.size() > 0) {
            ...
            GrammarEnvironment env =
                new GrammarEnvironment(new GrammarInputPane(myOriginalGrammar));
                ...
                for (int j = 0; j < p2.length; j++) {
                    ...
                    if (!p2[j].equals(p[i])) {
                        ...
                        ArrayList<String> variables =
                            getDifferentVariable(p[i].getRHS(), p2[j].getRHS());
                        ...
                        for (int pp = 0; pp < variables.size(); pp++) {
                            if (directLambdaProductions.containsKey(variables.get(pp))) {
                                ...
                                if (indirectLambdaProductions
                                    .containsKey(variables.get(pp))) {
    ...
    private void intializeUnitStepMap(Grammar g) {
        ...
        if (remover.getUnitProductions(g).length > 0) {
            ...
            Grammar unitless = remover.getUnitProductionlessGrammar(controller.getGrammar(),
                remover.getVariableDependencyGraph(g));
