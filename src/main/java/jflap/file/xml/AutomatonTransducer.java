...
public abstract class AutomatonTransducer extends AbstractTransducer {
    ...
    protected void createState(ArrayList<Node> stateNodes, Map<Integer, Node> i2sn,
                               Automaton automaton, Set<Object> locatedStates,
                               Map<Integer, State> i2s, boolean isBlock,
                               Document document) {
...
//        assert(automaton instanceof TuringMachine); //this code should really be in TMTransducer, but I see why it's here
    ...
    protected abstract Transition createTransition(State from, State to,
                                                   Node node, Map<String, String> e2t,
                                                   boolean isBlock);
...
//		}
    ...
    private Element writeFields(Document doc, Automaton auto, Element se) {
        ...
        if (auto instanceof TuringMachineBuildingBlocks) { //there should not be building blocks in non-Turing Machines
            Map<String, TuringMachineBuildingBlocks> references =
                ((TuringMachineBuildingBlocks) auto).getTMBBBlockMap();
