        ...
        if (Universe.CHOOSER != null) {
            ...
            if (environment instanceof AutomatonEnvironment) { //this is strictly for non-Grammar
                ...
                if (new File("svg.jar").exists() ||
                    ...
                    try {
                        RestrictedAction ra =
                            (RestrictedAction) Class.forName("jflap.gui.action.ExportAction")
...
//					(jflap.gui.environment.GrammarEnvironment) environment));
    ...
    private static JMenu getViewMenu(EnvironmentFrame frame) {
        ...
        if (AutomatonAction.isApplicable(object)) {
            ...
            subMenu
                .add(new LayoutAlgorithmAction("Diagonal From Upper-Left To Lower-Right", automaton,
                    environment, VertexMover.POSITIVE_SLOPE_DIAGONAL));
            subMenu
                .add(new LayoutAlgorithmAction("Diagonal From Lower-Left To Upper-Right", automaton,
...
//			menu.add(new StateColorSelector(automaton,environment,menu));
    ...
    private static JMenu getTestMenu(EnvironmentFrame frame) {
        ...
        if (GrammarTypeTestAction.isApplicable(object)) {
            addItem(menu,
                new GrammarTypeTestAction((jflap.gui.environment.GrammarEnvironment) environment));
        }
        return menu;
    }
