...
public class TwoCircleLayoutAlgorithm extends LayoutAlgorithm {
    ...
    public void layout(Graph g, Set<Object> notMoving) {
        ...
        innerCircle =
            innerCircleChain.getVertices();  //getting correct order for outer circle chains
    ...
    protected void createOuterCircleChains() {
        ...
        min = 0;
        for (int i = 0; i < outerCircleChains.length; i++) {
            if (outerCircleChains[min].getVertices().size() >
                outerCircleChains[i].getVertices().size()) {
                min = i;
            }
        }
        for (int i = 0; i < outerCircle.size(); i++) {
