...
public class Automaton implements Serializable, Cloneable {
     ...
     */
    private HashMap<State, LinkedList<Transition>> transitionFromStateMap =
        new HashMap<State, LinkedList<Transition>>();
    /**
     * A mapping from state to a list holding transitions to those states.
     */
    private HashMap<State, LinkedList<Transition>> transitionToStateMap =
        new HashMap<State, LinkedList<Transition>>();
    /**
     * A mapping from states to an array holding transitions from a state. This
     * is a sort of cashing.
     */
    private HashMap<State, Transition[]> transitionArrayFromStateMap =
        new HashMap<State, Transition[]>();
    /**
     * A mapping from states to an array holding transitions from a state. This
     * is a sort of cashing.
     */
    private HashMap<State, Transition[]> transitionArrayToStateMap =
        new HashMap<State, Transition[]>();
    ...
    private final transient HashSet<AutomataTransitionListener> transitionListeners =
        new HashSet<AutomataTransitionListener>();
    private final transient HashSet<AutomataStateListener> stateListeners =
        new HashSet<AutomataStateListener>();
    private final transient HashSet<AutomataNoteListener> noteListeners =
        new HashSet<AutomataNoteListener>();
    ...
    public static void become(Automaton dest, Automaton src) {
        ...
        while (it.hasNext()) {
            ...
            for (int i = 0; i < ts.length; i++) {
                ...
                Transition toBeAdded = (Transition) ts[i]
                    .clone(); //call clone instead of copy so that the jflap.gui stuff can get appropriately updated
...
//				a.addTransition(ts[i].copy(from, to));
    ...
    public void replaceTransition(Transition oldTrans, Transition newTrans) {
        ...
        List<Transition> list =
            transitionFromStateMap.get(oldTrans.getFromState());
...
//	}
     ...
     *
     * @param in the read stream for the object
     * @deprecated
     */
